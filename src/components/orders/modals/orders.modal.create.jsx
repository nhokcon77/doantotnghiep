...
import {updateTypeOrder, createOrder} from '../../../redux/orders/orders.thunk';
...
export function OrderCreateModal({ title, visible, onCancel,okText="Ok",cancelText="Hủy" })
{
	...
	const handleUpdateOrder = () => {
		...
		if(form.getFieldValue("type") !== "CONFIRM")
		{
			const data = {
				id:order.id,
		...
		form.validateFields().then((values) => {
			...
			bodyJson.saleNote = values.saleNote
			...
			bodyJson.shippingType = values?.shippingType || "GHTK"
			dispatch(createOrder(bodyJson))
			onCancel()
		}).catch(console.log)
	}
	...
	const searchWithWard = debounce((term) => {
		if(location.selectedDistrict && location.selectedDistrict)
		{
					...
					<div className="row flex-col w50">
						...
						<div className='flex between'>
							<Form.Item name="province" className='minw200'  label="Tỉnh/thành phố" rules={[{ required: true, message: 'Vui lòng chọn tỉnh/thành phố' }]}>
								<AutoComplete options={location?.province || []}  name="province"  onSelect={(e,j)=> OnSelectLocation(j,"province")}>
						...
						<div className='flex flex-col between bbottom pbottom'>
							<div className="flex flex-col between">
								<Form.Item name="isFreeShip" label="FreeShip">
									<Switch name="isFreeShip"   onChange={setIsFreeShip} />
								</Form.Item>			
							</div>
							<div className="flex flex-col between">
							...
							<div className="flex flex-col between">
								<Form.Item name="shippingType" label="Đơn vị vận chuyển">
									<Radio.Group defaultValue={"GHTK"} optionType="button" buttonStyle="solid" name="shippingType" options={optionShip} />
								</Form.Item>			
							</div>		
							<div  className="flex flex-col between">
